...
const ProfileFocusArea = ({ data = [] }) => {
    ...
    const handleSave = async () => {
        try {
            await updateMission({ focusArea: areas }).unwrap();
            setSaved([...areas]);
            setIsEditing(false);
            setInputValue('');
            toast.success('Domaines enregistrés');
        } catch (error) {
            ...
            <div className="">
                <div className="flex items-center justify-between mb-5">
                    <div className="flex items-center gap-2">
                        ...
                        <h1 className="text-lg font-bold text-indigo-800">Domaines d’accompagnement</h1>
                    </div>
                    {isEditing ? (
                ...
                <div className="flex flex-wrap gap-2">
                    ...
                    {areas.length === 0 && (
                        <p className="text-sm text-gray-400 py-2">
                            Aucun domaine pour le moment. Cliquez sur Modifier pour en ajouter.
                        </p>
                    )}
                </div>
            </div>
