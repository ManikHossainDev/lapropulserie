...
export class GenericController<ModelType, InterfaceType> {
  ...
  create = catchAsync(async (req: Request, res: Response) => {
    ...
    const videoUploadTokens = data.__videoUploadTokens as VideoUploadTokenMap | undefined;
    delete data.__videoUploadTokens;

    const result = await this.service.create(data);

    if (videoUploadTokens && result && (result as any)._id && req.stagedVideoUploads) {
      for (const [fieldName, token] of Object.entries(videoUploadTokens)) {
        if (!token) {
          continue;
        }

        const stagedUpload = req.stagedVideoUploads[token];
        if (!stagedUpload) {
          continue;
        }

        await enqueueVideoProcessingJob(stagedUpload, {
          targetModel: this.modelName as VideoTargetModelName,
  ...
  createWithAttachments = catchAsync(async (req: Request, res: Response) => {
    ...
    if ((req.files as any) && (req.files as any).attachments) {
    attachments.push(
        ...(await Promise.all(
        (req.files as any).attachments.map(async (file: any) => {
  ...
  getAllV2 = catchAsync(async (req: Request, res: Response) => {
    ...
    const result = await this.service.getAllV2(filters, populateOptions as any[], select);
  ...
  getByIdV2 = catchAsync(async (req: Request, res: Response) => {
    ...
    const result = await this.service.getById(id, populateOptions as any[], select);
    if (!result) {
      throw new ApiError(
        StatusCodes.NOT_FOUND,
        `Object with ID ${id} not found`
  ...
  updateById = catchAsync(async (req: Request, res: Response) => {
    ...
    if (videoUploadTokens && req.stagedVideoUploads) {
      for (const [fieldName, token] of Object.entries(videoUploadTokens)) {
        if (!token) {
          continue;
        }
  ...
  updateWithImageById = catchAsync(async (req: Request, res: Response) => {
    ...
    const existingObjectDTO : any = {
      attachments : (req.uploadedFiles || [] as any).attachments?.[0] ?? (existingObject as any)?.attachments,
      ...req.body
    }
