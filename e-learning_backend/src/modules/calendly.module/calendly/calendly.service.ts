...
// Create webhook subscription
  async createWebhookSubscription(accessToken: string, userUri:string) {
    ...
    if (!webhookUrl) {
      console.warn('CALENDLY_WEBHOOK_URL not set, skipping webhook creation');
      return null;
    }
    ...
    try {
      ...
      if (status === 403) {
        console.warn('Webhook creation forbidden (403). User may need to upgrade. Continuing without webhook.');
        return null;
      }

      console.error('Webhook creation error:', data);
      return null;
    }
  }
