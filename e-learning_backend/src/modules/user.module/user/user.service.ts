...
import PaginationService, {
  normalizeAggregationDocument,
} from '../../../common/service/paginationService';
...
} from '../../wallet.module/walletTransactionHistory/walletTransactionHistory.constant';
...
export class UserService extends GenericService<typeof User, IUser> {
  ...
  private buildAdminUserManagementMatchStage(
    filters: IUserManagementFilters,
  ): Record<string, any> {
    const matchStage: Record<string, any> = {
      role: { $in: ['student', 'mentor', 'admin'] },
    };
    ...
    if (filters.journeyType) {
      matchStage.journeyType = filters.journeyType;
    }
    ...
    if (filters.from) {
      createdAtRange.$gte = new Date(filters.from);
    }

    if (filters.to) {
      createdAtRange.$lte = new Date(filters.to);
    }
  ...
  private getAdminUserProfileLookupStages() {
    return [
      {
        $lookup: {
          from: 'userprofiles',
  ...
  private buildAdminUserProjection(includeAcceptTOC = false) {
    return {
      $project: {
        _id: 1,
        name: 1,
        ...
        profile: {
          ...
          ...(includeAcceptTOC
            ? { acceptTOC: '$profileInfo.acceptTOC' }
            : {}),
        },
      },
    };
  ...
  async getAdminUserManagementList(
    filters: IUserManagementFilters,
    options: PaginateOptions,
  ) {
    const matchStage = this.buildAdminUserManagementMatchStage(filters);
    const sortStage = PaginationService.buildSortObject(
      typeof options.sortBy === 'string' ? options.sortBy : undefined,
    );

    const pipeline = [
      { $match: matchStage },
      ...this.getAdminUserProfileLookupStages(),
      { $sort: sortStage },
      this.buildAdminUserProjection(),
    ];

    return PaginationService.aggregationPaginate(User, pipeline, options);
  }

  async getAdminUserManagementDetails(id: string) {
    const user = await User.aggregate([
      ...
      ...this.getAdminUserProfileLookupStages(),
      this.buildAdminUserProjection(true),
    ]);
    ...
    return normalizeAggregationDocument(user[0]);
  }

  async updateAdminManagedUserStatus(id: string, status: TStatusType) {
  ...
  async updateAdminManagedUserJourneyType(
    id: string,
  ...
  getProfileInformationOfAUser = async (loggedInUser: IUserFromToken) => {
    ...
    const user = await User.findById(id)
      .select('name email phoneNumber profileImage calendly')
      .lean();
    const userProfile = await UserProfile.findOne({ userId: id })
      .select('location dob gender')
      .lean();

    // Do not leak raw access/refresh tokens to the client, only expose connection status.
    const isCalendlyConnected = Boolean(
      user?.calendly?.encryptedAccessToken && !user?.calendly?.disconnectedAt,
    );
    ...
    return {
      ...
      calendly: {
        ...
        connectedAt: user?.calendly?.connectedAt || null,
        disconnectedAt: user?.calendly?.disconnectedAt || null,
        profileUrl: user?.calendly?.profileUrl || null,
      },
    };
  };

  //☑️☑️☑️☑️☑️☑️☑️
  getEarningAndCategoricallyBookingCountAndRecentJobRequest = async (
    providerId: string,
    type: string,
  ) => {
    ...
    if (type === 'weekly') {
      ...
      dateLabels = Array.from({ length: daysInMonth }, (_: any, idx: number) => `${idx + 1}`);
    } else {
    ...
    if (type === 'weekly') {
      ...
      chartData.forEach((_: any, idx: number) => {
  ...
  async getAllWithAggregationWithStatistics_V2_ProviderCountFix(
    ...
    async function calculateCurrentAndLastMonthsUserCountByRole(role: string) {
      ...
      const [allCount, thisMonthEarnings, lastMonthEarnings] =
        await Promise.all([
          // All Count
          User.aggregate([
            { $match: { ...baseQuery } },
    ...
    async function calculateCurrentAndLastMonthsServiceBookingCount() {
      ...
      const [allBookingCount, thisMonthBooking, lastMonthBooking] =
        await Promise.all([
          // This month earnings
          (ServiceBooking as any).aggregate([
            { $match: { ...baseQuery } },
            {
              $group: {
                _id: null,
    ...
    const walletIdOfUser: IUser =
      await User.findById(userId).select('walletId');
    ...
    async function getTotalRevenueByMonths(year: string) {
      ...
      const totalTransactionsAmountForAdmin =
        await WalletTransactionHistory.aggregate([
          {
            $match: {
              walletId: walletIdOfUser.walletId,
              isDeleted: false,
      ...
      totalTransactionsByMonth.forEach((item: any) => {
      ...
      return {
        monthlyData: result,
        totalTransactionsAmountForAdmin:
          totalTransactionsAmountForAdmin[0]?.total || 0,
    ...
    // get BookingCount
    const bookingCount = await ServiceBooking.aggregate(
      serviceBookingStatPipeline,
    );
    ...
    const totalTransactionAmountForAdmin = await PaymentTransaction.aggregate(
      paymentTransactionStatPipeline,
    );
    ...
    const statistics = {
      ...
      totalProviders:
        acceptedProvidersCount[0]?.totalAcceptedProviders + 1 || 0, // hotfix: only shows thoses providers who's status is accept

      totalSubAdmin:
        roleStats.find(stat => stat._id === 'subAdmin')?.count || 0,
      totalAdmin: roleStats.find(stat => stat._id === 'admin')?.count || 0,
      totalServiceBooking: bookingCount[0]?.count || 0,
      totalTransactionAmountForAdmin:
        totalTransactionAmountForAdmin[0]?.total || 0,
    };
  ...
  updateProfileInformationOfAUser = async (
    id: string,
    ...
    if (data.dob) profileUpdateData.dob = data.dob;
    if (data.gender) profileUpdateData.gender = data.gender;
    if (data.location) profileUpdateData.location = data.location;
    ...
    const updatedProfile = await UserProfile.findOne({ userId: id }).lean();
  ...
  updateProfileInformationOfAdmin = async (
    id: string,
    data: IUpdateUserInfo,
  ) => {
  ...
  async updateProfileImageSeperately(userId: string, data: any): Promise<any> {
    ...
    console.log(
      'data service  data?.profileImage[0] -> ',
      data?.profileImage[0],
    );
    ...
    if (!data?.profileImage[0]) {
      throw new ApiError(
        ...
        'You have to upload an image to update',
      );
    }
  ...
  //---------------------------------
  async getAllWithAggregationV2(filters: any, options: PaginateOptions) {
    /*-----------------------------------------   
   ...
   // TODO : MUST : created At filter add korte hobe ..
    ...
    for (const key in filters) {
      ...
      // --- Match for Users collection ---
      if (
        ['name', 'email', 'phoneNumber', 'role', '_id', 'from', 'to'].includes(
          key,
        )
      ) {
        if (key === 'name') {
      ...
      // --- Match for userroledatas (joined collection) ---
        ...
        if (Array.isArray(value)) {
          roleDataMatchStage['userRoleDataInfo.providerApprovalStatus'] = {
            $in: value,
          };
        } else {
    ...
    const pipeline = [
      ...
      ...(Object.keys(roleDataMatchStage).length > 0
        ? [{ $match: roleDataMatchStage }]
        : []),
