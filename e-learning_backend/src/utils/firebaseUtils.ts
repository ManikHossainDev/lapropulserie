...
export const sendPushNotificationV2 = async (
  fcmToken: string,
  messageData: any | string, // Can accept object or stringified JSON
  ...
  try {
    ...
    if (
      ...
      if (!isValidUrl(imageUrl)) {
        parsedMessageData.image.imageUrl =
          'https://newsheakh6737.sobhoy.com/uploads/users/user.png';
      }
    }

    // const {message, notificationTitle} = await buildFCMMessageV2(messageData as IMessageToEmmit , fcmToken);

    const { message, notificationTitle } = await buildFCMMessageV2(
      parsedMessageData,
      fcmToken,
    );
    ...
    throw new Error(
      `Error sending push notification: ${error.message || error}`,
    );
  }
};

export const buildFCMMessageV2 = async (
  messageData: any | string,
  fcmToken: string,
): Promise<{
...
const buildFCMMessage = async (
  ...
  console.log(
    'Preparing to send push notification V2 with message:',
    parsedMessage,
  );
