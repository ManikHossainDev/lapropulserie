...
export class RedisStateManager {
  ...
  private readonly KEYS = {
    ...
    USER_SOCKET_MAP: 'user_socket_map:', // Hash: userId -> connection info
    SOCKET_USER_MAP: 'socket_user_map:', // Hash: socketId -> userId
    USER_ROOMS: 'user_rooms:', // Set: userId's joined rooms
    ROOM_USERS: 'room_users:', // Set: room's connected users
    USER_STATUS: 'user_status:', // Hash: userId -> status
  };
  ...
  async handleUserReconnection(
    ...
    if (existingInfo && existingInfo.socketId !== newSocketId) {
      ...
      await this.redis.del(
        `${this.KEYS.SOCKET_USER_MAP}${existingInfo.socketId}`,
      );
  ...
  async addOnlineUser(
    ...
    if (typeof userId !== 'string') {
      logger.error('❌ userId is not a string:', {
        type: typeof userId,
        value: userId,
      });
    ...
    if (typeof socketId !== 'string') {
      logger.error('❌ socketId is not a string:', {
        type: typeof socketId,
        value: socketId,
      });
  ...
  async getUserBySocketId(socketId: string): Promise<string | null> {
    const userId = await this.redis.hGet(
      `${this.KEYS.SOCKET_USER_MAP}${socketId}`,
      'userId',
    );
    return userId ?? null;
  }
