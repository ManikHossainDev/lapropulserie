...
const redisHost = process.env.REDIS_HOST || config.redis.host || 'redis';
const redisPort = parseInt(process.env.REDIS_PORT || config.redis.port || '') || 6379;

// BullMQ uses ioredis internally and expects a plain { host, port } object
// (or an ioredis instance) — NOT the node-redis v4 client's `.options` shape
// (which is `{ url, socket }` and has no top-level host/port). Passing
// `redisPubClient.options` to BullMQ silently falls back to 127.0.0.1:6379.
