...
/**
 * Accepts both Mongoose syntax ('-createdAt') and the `field:asc,field:desc`
 * syntax the clients send. Without this, `capsuleNumber:asc` is treated as an
 * unknown field name and the results come back unsorted.
 */
const parseSortBy = (sortBy?: string): string | Record<string, 1 | -1> => {
  ...
  return Object.keys(sort).length ? sort : 'createdAt';
};
