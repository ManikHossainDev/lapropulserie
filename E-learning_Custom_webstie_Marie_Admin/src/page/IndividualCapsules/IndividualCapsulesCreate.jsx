...
import { useCreateIndividualcapsuleMutation } from '../../redux/features/Individualcapsule/Individualcapsule';
...
const IndividualCapsulesCreate = () => {
  const navigate = useNavigate();
  const [createIndividual, { isLoading }] = useCreateIndividualcapsuleMutation();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await createIndividual(formData).unwrap();
      toast.success('Capsule category created successfully!');
      navigate(-1);
    } catch (error) {
  ...
  return (
    <div className="min-h-screen p-5">
      <div onClick={() => navigate(-1)} className="flex items-center gap-1 cursor-pointer mb-7 w-fit">
        ...
        <h1 className="text-xl font-bold text-[#1a1a2e]">Create Capsule Category</h1>
      </div>

      <div className="bg-white rounded-2xl border border-[#eaecf4] shadow-sm max-w-3xl">
        ...
        <form onSubmit={handleSubmit} className="px-6 py-6 flex flex-col gap-6">
          ...
          <div className="flex items-center justify-end gap-3 pt-2">
            <button
              ...
              onClick={() => navigate(-1)}
              disabled={isLoading}
              className="h-11 px-6 rounded-xl border border-[#eaecf4] bg-white text-sm font-semibold text-[#7b7d9d] hover:border-[#2d2a71] hover:text-[#2d2a71] transition-colors disabled:opacity-50"
            >
              Cancel
            </button>
            <button
...
export default IndividualCapsulesCreate;
