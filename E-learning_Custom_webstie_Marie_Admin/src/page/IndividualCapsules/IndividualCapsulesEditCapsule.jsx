      ...
      {mode === 'upload' ? (
        uploadPreview ? (
          <div className="relative rounded-xl overflow-hidden border border-[#eaecf4] h-36">
            ...
            <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/50 to-transparent px-3 py-2">
              <p className="text-white text-xs truncate">{fileState?.name || 'Current uploaded video'}</p>
            </div>
          </div>
        ) : (
          <label
        ...
        <div className="flex flex-col gap-2">
          ...
          {embedState && /youtube|vimeo|youtu\.be/i.test(embedState) && (
            <p className="text-[11px] text-green-700">Embed link detected — will replace any uploaded file on save.</p>
          )}
        </div>
      )}
    </div>
